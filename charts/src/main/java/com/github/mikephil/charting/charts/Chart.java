...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
     ...
     * @param
     */
    public void highlightValues(ArrayList<Integer> content, int dataSetIndex) {
        Highlight[] array = new Highlight[content.size()];
        int c = 0;


        for (Integer l : content) {
            Entry e= getData().getDataSetByIndex(dataSetIndex).getEntryForIndex(l);
            array[c] = new Highlight(e.getX(), e.getY(), dataSetIndex);
            c++;
        }


        // set the indices to highlight
        mIndicesToHighlight = array;
     ...
     * @param exclude
     */
    public void highlightAllValues(ArrayList<Integer> exclude) {
        ArrayList<Highlight> array = new ArrayList<Highlight>();

        for(int j=0; j<getData().getDataSetCount(); j++){

            for(int k=0; k<getData().getDataSetByIndex(j).getEntryCount(); k++){

                Entry e= getData().getDataSetByIndex(j).getEntryForIndex(k);
                if(e!=null && !exclude.contains(k))
                array.add(new Highlight(e.getX(), e.getY(), j));
            }
        ...
        // set the indices to highlight
        mIndicesToHighlight = array.toArray(new Highlight[array.size()]);
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        Log.d("drawMarkers", "DRA:" + isDrawMarkersEnabled() + "  " + !valuesToHighlight() + "  " + mMarker);
        // if there is no marker view or drawing marker is disabled
        if (mMarker == null || !isDrawMarkersEnabled() || !valuesToHighlight())
            return;
        Log.d("drawMarkers", "DRA2:" + mIndicesToHighlight.length);
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            // draw the marker
            mMarker.draw(canvas, pos[0], pos[1]+30);
        }
    }
