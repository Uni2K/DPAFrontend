...
public class CustomBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public CustomBottomSheetBehavior() {        setHideable(true);
    }
    public void setNestedScrollingChildRef(View v) {
        this.nestedScrollingChildRef = new WeakReference<View>(v);

    }
    public WeakReference<View> getNestedScrollingChildRef(View v) {
    ...
    public CustomBottomSheetBehavior(Context context, AttributeSet attrs) {
        ...
        if (hasBackgroundTint) {
           ColorStateList bottomSheetColor =ColorStateList.valueOf(a.getColor(R.styleable.BottomSheetBehavior_Layout_backgroundTint,0));
    ...
    private void updateDrawableOnStateChange(@State int state, @State int previousState) {
        if (materialShapeDrawable != null) {
            // If the CustomBottomSheetBehavior's state is set directly to STATE_EXPANDED from
    ...
    void startSettlingAnimation(View child, int state) {
        ...
        if (state == STATE_COLLAPSED) {
            ...
            throw new IllegalArgumentException("Illegal state argument: " + state+"  "+hideable);
        }
        if (viewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
            setStateInternal(STATE_SETTLING);
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcelable superState, CustomBottomSheetBehavior behavior) {
            super(superState);
